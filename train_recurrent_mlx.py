...
class Hyperparameters:
    ...
    # Model (depth-recurrent architecture with SwiGLU).
...
class Block(nn.Module):
    # Shared transformer block. Per-loop specialization happens via external gate scalars,
    # not internal state — so the same block can behave differently at each virtual depth.
...
class GPT(nn.Module):
    # Depth-recurrent transformer: num_unique_blocks shared blocks repeated num_repeats times.
    # Per-virtual-layer gate scalars and resid_mix provide specialization without duplicating blocks.
    # Informed by Two-Scale Latent Dynamics (ICLR 2026): loops learn orthogonal refinements.
    def __init__(self, vocab_size: int, num_unique_blocks: int, num_repeats: int, dim: int,
        ...
        # These are cheap (2*dim + dim + dim per layer) but let each virtual layer specialize.
        self.layer_attn_scales = [mx.ones((dim,), dtype=mx.float32) for _ in range(self.num_virtual_layers)]
        self.layer_mlp_scales = [mx.ones((dim,), dtype=mx.float32) for _ in range(self.num_virtual_layers)]
        self.layer_resid_mixes = [
            mx.array(np.stack((np.ones((dim,), dtype=np.float32), np.zeros((dim,), dtype=np.float32))))
            for _ in range(self.num_virtual_layers)
        ]

        # Zero-init output projections for stable residual start.
    ...
    def __call__(self, input_ids: mx.array) -> mx.array:
        ...
        # Depth recurrence: cycle through shared blocks, each virtual layer gets its own gates.
...
class SplitOptimizers:
    ...
    # - block scalars + per-layer gates: Adam
...
def main() -> None:
    ...
    log(
        ...
        f"unique_blocks:{args.num_unique_blocks} repeats:{args.num_repeats} "
        f"virtual_layers:{args.num_unique_blocks * args.num_repeats} "
